//【控件初始化】    
    ...
    $("#search_manager").dropdown({
        apiSettings:    { url: '/stationconfig/manager/query/json', cache:false },
        saveRemoteData: false,
    }); //搜索模块：管理人下拉框AJax加载；

    $("#search_fund").dropdown({
        apiSettings: { url: '/stationconfig/fund/query/json/', cache:false },
        saveRemoteData: false,
    }); //搜索模块：项目名称下拉框AJax加载；
    ...
    //【事件】
      ...
      $("#search_manager").dropdown('setting', 'onChange', function(value, text){
      	$("#search_fund").dropdown({
      		apiSettings: { url: '/stationconfig/fund/query/json/'+value, cache:false},
          });
      	$("#search_fund").dropdown('clear');
      }); //搜索模块：管理人和项目名称下拉框联动；

      $("#add_manager_exist").dropdown('setting', 'onChange', function(value, text){ 
          var id = $('#add_manager_exist').dropdown('get value');
          alert(id);
          $.ajaxSetup({ data: {csrfmiddlewaretoken: '{{ csrf_token }}' },   });
          $.ajax({    
            type:'post',        
            url:'/stationconfig/select/manager',    
            data:{'manager_id':id},   
            cache:false,    
            dataType:'json',    
      ...
      $("#add_fundtype").dropdown('setting', 'onChange', function(value, text){ 
          var history = $('#add_history').dropdown('get value');
          if (value=='xbrl'){
            if (history=='True'){
      ...
      $("#add_history").dropdown('setting', 'onChange', function(value, text){ 
      ...
      $("#modify_manager").dropdown('setting', 'onChange', function(value, text){ 
      ...
      $("#modify_fundtype").dropdown('setting', 'onChange', function(value){ 
